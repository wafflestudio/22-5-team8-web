    ...
    <div className="flex-none w-[100px] snap-start">
      <div className="flex flex-col gap-1">
        <div className="relative">
          ...
          <img src={movie.poster_url} className="w-full object-cover rounded-sm" />
        </div>
        <h1 className="text-sm truncate">{movie.title}</h1>
        <h1 className="text-gray-500 text-xs">평균★ {movie.average_rating === null ? '별점 없음' : movie.average_rating.toFixed(1)}</h1>
      </div>
    </div>
  );
};
